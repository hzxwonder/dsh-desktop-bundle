...
// ---------- 宿主存活感知（防孤儿进程） ----------
//
// Helper 由宿主（DSH）spawn，与宿主之间只有 stdout/stderr/stdin 管道。宿主退出后若本进程
// 继续存活：① 渲染端的 bridge 请求仍在向已断开的 stdout 写行 → EPIPE 成为未捕获异常，弹
// 「A JavaScript error occurred in the main process」对话框；② 宠物窗口不关闭成为孤儿，
// 下次宿主启动再拉起一只就出现两只。所以「宿主没了」直接作为退出条件，任一信号触发即退：
//   - stdout/stderr 写失败（EPIPE 等）：error 处理同时把流错误从「未捕获异常」降级为退出信号；
//   - stdin EOF：宿主进程消失后管道写端关闭（仅 stdin 真的是宿主管道时监听；
//     手动 start-desktop 的 inherit/TTY 与 DPI 探测的 /dev/null 都不是宿主管道，不能秒关即退）；
//   - 父进程消亡轮询：Unix 父死后 ppid 被 init/launchd 收养而变化，Windows ppid 不变但
//     kill(ppid, 0) 报 ESRCH，两路合一覆盖全平台（EPERM = 仍在，只是探测无权限）。
let hostGone = false;
function quitWhenHostGone(reason) {
  if (hostGone) return;
  hostGone = true;
  console.error('[dsh-pet-desktop-helper] host is gone (' + reason + '); shutting down');
  for (const win of windows.values()) {
    if (!win.isDestroyed()) win.destroy();
  }
  // 退出推迟一拍，让日志先落盘（同 DPI 探测分支的做法）
  setTimeout(() => app.exit(0), 0);
}

process.stdout.on('error', () => quitWhenHostGone('stdout pipe closed'));
process.stderr.on('error', () => quitWhenHostGone('stderr pipe closed'));
if (!DPI_PROBE) {
  // stdin = 宿主管道（FIFO 或 socketpair——Node spawn 的 stdio 管道在 Unix 上是 socketpair）
  // 才监听 EOF；TTY（手动启动）与 /dev/null（探测）都不是宿主管道，不能秒关即退
  let stdinIsHostPipe = false;
  try {
    const stdinStat = require('node:fs').fstatSync(0);
    stdinIsHostPipe = stdinStat.isFIFO() || stdinStat.isSocket();
  } catch {
    /* fd 0 不可探测：不启用 EOF 信号，靠父进程轮询兜底 */
  }
  if (stdinIsHostPipe) {
    process.stdin.on('error', () => quitWhenHostGone('stdin pipe closed'));
    process.stdin.on('end', () => quitWhenHostGone('stdin EOF'));
    process.stdin.resume(); // 流动态才收得到 EOF 的 end；写入内容本进程从不消费（应答走 HTTP 回调）
  }
  const initialPpid = process.ppid;
  if (initialPpid && initialPpid !== 1) {
    setInterval(() => {
      if (process.ppid !== initialPpid) {
        quitWhenHostGone('orphaned (adopted by init)');
        return;
      }
      try {
        process.kill(initialPpid, 0); // 0 = 只探测存活，不发信号
      } catch (error) {
        if (error && error.code === 'ESRCH') quitWhenHostGone('parent process gone');
      }
    }, 2000).unref?.();
  }
}
...
function bridgeRequest(method, url, body) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // 管道已断（宿主退出）：降级为退出信号，绝不让 EPIPE 冒成未捕获异常弹框
      bridgePending.delete(id);
      quitWhenHostGone('stdout write failed');
