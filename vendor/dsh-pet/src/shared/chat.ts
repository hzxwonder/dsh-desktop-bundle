...
//  - 多会话粗略列表（workStatusEnabled 开启时）：打开时有 ≥2 条非 subagent 会话 →
//    输入框上方渲染「标题 + 任务 + 右侧状态符号」列表（sessions-panel，同一份 DOM），
//    弹窗开着期间 1s 轮询刷新、完成条目 10s 淡出；单会话保持最简样式（无列表）。
...
import { fetchWorkStatus } from './work-status';
import { createSessionsList } from './sessions-panel';

/** 多会话列表数据刷新节奏（弹窗开着期间） */
const SESSIONS_POLL_MS = 1000;
...
export const CHAT_CSS = [
  ...
  // 多会话列表与输入框的分隔（无列表时不占位：display:none 的兄弟不产生间距）
...
export function mountChatDialog(opts: {
  ...
  const workStatusUrl = opts.workStatusUrl === undefined ? '/dsh-pet-7340/work-status' : opts.workStatusUrl;
  ...
  // 多会话列表：≥2 条非 subagent 会话才启用（单会话保持最简样式）；启用前 hidden 不占位
  const sessionsList = createSessionsList();
  sessionsList.el.style.display = 'none';
  let sessionsOn = false;

  // 宽度/高度自适应：宽度按文本测量（≤CHAT_MAX_W），封顶后高度随折行增长；开列表时根宽不小于列表最小宽
  ...
  // 多会话列表刷新（弹窗开着期间）：过滤 subagent，≥2 条才显示；失败静默（列表是辅助信息，不影响输入）
  let sessionsTimer = -1;
