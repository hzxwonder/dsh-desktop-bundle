...
每篇论文根目录拥有独立的 `AGENTS.md`，可以在工作台文件栏直接打开、编辑并保存。新建、导入和打开已有论文时，插件为缺失的文件生成默认写作、审阅与 Overleaf 同步规则；已有文件保留原内容。不同论文分别维护自己的规则。导入项目若已有自定义文件，可按需补充 Overleaf 要求。

论文绑定会话的系统提示词要求 Agent 每轮操作前读取根目录 `AGENTS.md`。Harness 同时沿用原生工作区指令加载机制，全局及适用的上级指令继续生效。默认规则要求先检查 Git，再将每轮论文修改提交并推送到经核实的 Overleaf 项目，检查同步结果。此过程由 Agent 执行，手动保存和编译本身不触发 Git 操作。`AGENTS.md` 是项目文件，可随论文版本管理；其中仅应保存协作规则。
