...
论文绑定会话的系统提示词要求 Agent 在开始操作前检查当前目录的 Git 状态，每轮论文修改完成后提交并推送到经核实的 Overleaf 项目。提交范围限定为本轮论文修改，推送成功后才报告同步完成；缺少远端、凭据或遇到冲突时保留本地成果并报告阻碍。此功能由 Agent 执行，编辑器手动保存与编译本身不会触发 Git 操作。
