...
初始页右上角的「全局设置」管理主题和内部 AGENTS.md，保存指令后用于后续论文 Agent 调用。论文内左下角齿轮打开「论文设置」，选择编译器与编译主文件，设置按论文自动保存。内部指令适用于工作台内的论文会话。默认规则要求先检查 Git，再将每轮论文修改提交并推送到经核实的 Overleaf 项目，检查同步结果。此过程由 Agent 执行，手动保存和编译本身不触发 Git 操作。Harness 原生的全局和项目指令仍按其作用域生效。
