...
在论文对话中收到此类请求时，先用 `paper-workbench` 的 `list` 找到当前项目，再调用 `analyze` 启动本技能流程，并轮询 `job`。首次发起更新的论文会话会被标记为专属导图会话；工作台按钮始终复用该会话。整个流程由工作台执行，Agent 不手动改写或分批提交 TeX 文件。用户手动修改导图专用注释后只需调用 `rerender`，它重新解析已有注释，不调用模型。
