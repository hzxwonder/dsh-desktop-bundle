...
一套按固定版本装配的 DSH Desktop 环境：仓库里存放 14 个已发布的插件源码快照，
...
`dsh-pet` 固定为 0.2.11（代码 MIT，动画素材由 AI 生成、全流程可复现），默认启用，提供桌面桌宠：工作状态联动（运行/等你确认/完成/出错）、多会话列表与翻档提示音、碎碎念与对话。安装时经其 `prepare` 脚本从源码构建（TypeScript 工具链已随插件依赖声明）。快照取自上游仓库的 `dsh-pet/` 子目录（manifest 以 `subdir` 标注），commit 为 v0.2.11 之上的本地修复（helper 看门狗与会话面板），待上游合并后回移到上游 tag。源码和许可证保存在 `vendor/dsh-pet/`。
...
输出逐项确认：profile 三件套齐全、14 个插件全部从本仓库的 `vendor/` 解析、
